...
def Train_Evaluate_Model(data):
    ...
    featuresR =["StudyHours","Attendance","PreviousScore","AssignmentsCompleted","SleepHours"]
    ...
    Y_pred = model_full.predict(X=X_test)
    ...
    accuracyX_real = accuracyX(Y_pred,Y_test)
    print("Accuracy Of model is :\n",accuracy_real*100)

    
    cm = confusion_matrix(Y_test,Y_pred,labels=model_full.classes_)
    ...
    Y_t_pred=model_full.predict(X_train)
...
###########################################
    ...
    new_predict = model_full.predict(new_data)
...
###########################################
    importance_model =model_full.feature_importances_

    for i in range(len(importance_model)):
        print(f"feature :{featuresR[i]}\t ,importane = {importance_model[i]}\n")
        if(importance_model[i]==1):
            print(f"{featuresR[i]} contributes most in Final Result prediction ")
...
###########################################################################
    ...
    print("Accuracy os real model is ",accuracy_real)
    print('Accuracy of user defined function is :',accuracyX_real)
...
##################################################################
    ...
    accuracy_real10= accuracy_score(y_true=Y_test,y_pred=Y_pred)
    accuracyX_real = accuracyX(Y_pred,Y_test)
...
##################################################################
    ...
    accuracy_real0= accuracy_score(y_true=Y_test,y_pred=Y_pred)
    accuracyX_real = accuracyX(Y_pred,Y_test)
    

    print("Accuracy os real model with random state = 42  is ",accuracy_real*100)
    print("Accuracy Of model with random state =0 is :\n",accuracy_real0*100)
    print("Accuracy Of model with random state =10  is :\n",accuracy_real10*100)

    plt.figure(figsize=(7, 5))
    
    plot_tree(
    model_full,
    feature_names=["StudyHours","Attendance","PreviousScore","AssignmentsCompleted","SleepHours"],
    ...
    print("Tree depth:", model_full.get_depth())
    print("Number of leaves:", model_full.get_n_leaves())

###################################################################3
#   Insert new column 
#####################################################################
    data["PerformanceIndex"]=(data["StudyHours"]*2) + data["Attendance"]
    print("Column 'Performance Index' added succesfully")

    print(data[["StudyHours", "Attendance", "PerformanceIndex"]].head())
...
###########################################################################
    ...
    Model = Model.fit(X_trainN,Y_trainN)

    Y_train_pred=Model.predict(X_trainN)
    Y_predicted = Model.predict(X_testN)

    Trainning_Acc = accuracy_score(Y_trainN,Y_train_pred)
    Testing_Acc = accuracy_score(Y_testN,Y_predicted)
    print("Trainning accuracy :",Trainning_Acc)
    print("Testing accuracy :",Testing_Acc)

    
def accuracyX(y_pred, y_test):
    T_p = 0
