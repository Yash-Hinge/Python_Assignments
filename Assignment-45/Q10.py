import pandas as pd
import matplotlib.pyplot as plt
import numpy as np
from sklearn.preprocessing import OneHotEncoder

def data():
    data =pd.DataFrame(
        {'Name':['Amit','Sagar','Pooja'],
         'Math':[85,90,78],
         'English':[75,85,82],
         'Science':[92,88,80]})

    border = "-"*50

    print(border)
    print("General Description :-")
    print(border)         

    print(border)
    print("Shape of DataFrame:")
    print(border)
    print(data.shape)
    print("\n")
    print(border)
    print("Column Name :-")
    print(list(data.columns))
    print(border)
    print("\n")
    print(border)
    print("Datatypes of Columns :-")
    print(data.dtypes)
    print(border)

    print(border)
    print("Detailed Description:- ")
    print(border)
    print(data.describe())
    print("\n")
    data['Total']= (data['Math']+data['English']+data['Science'])
    print(border)
    print("Updated DataFrame :-")
    print(border)
    print(data)
    print("\n")

    print(border)
    print("Score in Science greater than 85 :- ")
    print(border)

    print(data[data['Science']>85])

    
    print(border)
    print("Updated Data:-")
    print(border)

    data['Name']=data['Name'].replace("Pooja","Puja")
    print(data)


    print(border)
    print("Sorted Data in Descending Order by total-")
    print(border)

    print(data.sort_values('Total',ascending=False))
          
    plt.bar(data['Name'],data['Total'],width=0.5)
    plt.xlabel("Student Name")
    plt.ylabel("Total Marks")
    plt.title("Total Marks v/s Student Name ")
    plt.show()

    Student = data[data['Name']=='Amit']
    Subject_performance = ["Math","English","Science"]

    plt.plot(Subject_performance,Student[Subject_performance].iloc[0],marker='o')
    plt.xlabel("Subjects")
    plt.ylabel("Performance in subjects ")
    plt.title("Amit's Performance ")
    plt.show()


    data2 =pd.DataFrame(
            {'Name':['Amit','Sagar','Pooja'],
             'Math':[np.nan,90,78],
             'English':[75,85,82],
             'Science':[92,np.nan,80]})

    print(border)
    print("New DataFrame ;-")
    print(border)
    print(data2)

    print(border)
    print("New Updated DataFrame :-")
    print(border)
    data2 = data2.fillna(data2.mean(numeric_only=True))
    print(data2)

    print(border)
    print("Dropped Column 'English' :-")
    print(border)

    data2 = data2.drop("English",axis =1)
    print(data2)

    print(border)
    print("Normalized Math Column Using min-max scaling :-")
    print(border)
    dataX=data.copy()
    dataX["Math"]=(dataX["Math"]-dataX["Math"].min())/(dataX["Math"].max()-dataX['Math'].min())
    print(dataX)

    print(border)
    print("Insert 'Gender' column")
    print(border)

    data['Gender']=["Male","Male","Female"]
    print(data)

    print(border)
    print("Encoded 'Gender' Column")
    print(border)

    encoder = OneHotEncoder(drop="first",sparse_output=False)
    data['Gender']= encoder.fit_transform(data[['Gender']])
    print(data)

    print(border)
    print("Group by 'Gender' column and average marks")
    print(border)
    average = data.groupby("Gender")['Math'].mean()
    print(average)
    average = data.groupby("Gender")['English'].mean()
    print(average)
    average = data.groupby("Gender")['Science'].mean()
    print(average)

    print(border)
    print("Pie Chart for Marks Scored By Sagar ")
    print(border)

    record = data[data["Name"] == "Sagar"].iloc[0]

    values = [
    record["Math"],
    record["Science"],
    record["English"]
    ]
    for v in values :
        print(v)

    plt.pie(
    values,
    labels=["Math", "Science", "English"]
    
    )

    plt.show()

    print(border)
    print("New Updated DataSet:- ")
    print(border)

    data['Status']= data["Total"].apply(lambda x:'Pass' if x>250 else'Fail')

    print(data)

    print(border)
    print("Passed Student Count :- ")
    print(border)
    PassCnt=0
    for d in data['Status']:
        if d =='Pass':
            PassCnt+=1

    print("No. of Students passed : ",PassCnt)

    print(border)
    print("export dataframe to .csv file :- ")
    print(border)
    data.to_csv("Student_data.csv",index=False)
    print("File Exported Succesfully....")

    print(border)
    print("Histogram of maths marks :- ")
    print(border)
    
    plt.hist(data["Math"],bins=5)
    plt.title("Score of Students in Maths .")
    plt.xlabel("Marks")
    plt.ylabel("frequency")

    plt.show()

    print(border)
    print("Rename math :- ")
    print(border)
    data.rename(columns={'Math':"Mathmatics"},inplace=True)
    print(data)


    print(border)
    print("Boxplot of English marks :- ")
    print(border)

    plt.boxplot(data['English'])
    plt.title("Boxplot of English Marks scored")
    plt.ylabel("Marks")
    plt.show()
def main():
    data()



if __name__ =="__main__":
    main()